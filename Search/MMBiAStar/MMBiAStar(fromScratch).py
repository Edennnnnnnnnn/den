...
class MMBiAStar:
	...
	def search(self) -> tuple:
		...
		"""
		# Initialization:
		self.initState.set_cost(self._getPValue(self.initState, self.goalState))
		self.goalState.set_cost(self._getPValue(self.goalState, self.initState))
		...
		while len(self.openF) > 0 and len(self.openB) > 0:
			...
			if self.openF[0].get_cost() < self.openB[0].get_cost():
				...
				for child in childrenValid:
					...
					child.set_cost(self._getPValue(child, self.goalState))
			...
			# Expanding Backward Search (exactly as above but with OPEN_b)
				...
				for child in childrenValid:
					# Computing the hash key and the f-value (g-value + h-value) for each node:
					childHashID = child.state_hash()
					child.set_cost(self._getPValue(child, self.initState))
	...
	def _getPValue(self, nodeLocal: State, nodeGoal: State) -> float:
		"""
			This is a helper function which is used to compute the P-value based on the given information;
		:params:
			- nodeLocal, State, the current agent location;
			- nodeGoal, State, the target location in searching;
		:returns:
			- pValue, float, the P-value computed based on the given information, where p(x) = max(F-Value, 2·G-Value);
		"""
		fValue = self._getFValue(nodeLocal, nodeGoal)
		pValue = max(fValue, 2 * nodeLocal.get_g())
		return pValue
